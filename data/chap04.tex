...
\subsection{聚类}
初始化之后，我们获得了元素的$ isCore $信息，以及元素之间的相邻关系$ u_{ij},i,j\in[n] $。接下来，我们将具体介绍一种基于DBSCAN的全新聚类方式。该算法的核心在于构造若干临时簇，通过记录临时簇之间的连接关系，最后由用户来还原最终簇划分结果。

若完全依据明文DBSCAN的方式来设计密文算法，受限于中间结果同样不能泄露的原则，方案的复杂度会非常高。具体流程
...
\begin{algorithm}[htbp]
	...
	\begin{algorithmic}[1]
		...
		\ENSURE 簇连接关系$ plist $
		\FOR{$ i=1 $ to $ n $}
		\STATE $d_i.isMark  \leftarrow  d_i.isCore+d_i.isMark - mul(d_i.isCore,d_i.isMark)$
		\ENDFOR
		\STATE $ s \leftarrow [0,...,0]_n $
		\FOR {$ j=1 $ to $ n $}
		\STATE $ m = mul(1-d_j.isMark, d_i.isCore) $
		\STATE $ m = mul(m, u_{ij}) $
		\STATE $ d_j.cluId \leftarrow d_j.cluId + mul(m, d_i.cluId - d_j.cluId) $
		\ENDFOR
	\end{algorithmic}
\end{algorithm}
