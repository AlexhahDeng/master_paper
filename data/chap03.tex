...
过程如算法\ref{alg_sf}所示。$ p_i $用于标识对于节点$ c $候选簇$ z_i $是否被剪枝，若$ p=1 $则$ z_i $仍为节点$ c $的候选簇，否则不是。
...
判断是否排除候选簇的具体计算方式为，首先将最近候选簇$ \langle  z^*  \rangle $与其他候选簇$ \langle  z_j  \rangle $按维度相减得到中间结果$ \langle  \mu  \rangle $，然后比较$ \langle  \mu  \rangle $每个维度与0的大小关系得到$ \langle  r  \rangle $，对于$ \langle  r  \rangle $中每个值，若小于0，则取当前树节点$ \langle  c  \rangle $中该维度数据的最小值，否则取最大值，得到$ \langle  v  \rangle $。
...
\begin{algorithm}[htbp]
	...
	\begin{algorithmic}[1]
		...
		\FOR {$j=1$ to $k$} %遍历每个簇
		\STATE $\langle r \rangle \leftarrow$ SC$(\langle u \rangle, 0)$
